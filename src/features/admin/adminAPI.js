...
export const getUsersAPI = async (page = 1, search = "", ordering = "") => {
  const res = await api.get(
    `admin/users/?page=${page}&search=${search}&ordering=${ordering}`,
  );
  return res.data;
};
...
export async function postAdminCancelOrder(orderNumber, reason = "") {
...
export async function fetchAdminOrder(orderNumber) {
  const res = await api.get(`admin/orders/${encodeURIComponent(orderNumber)}/`);
...
export async function fetchAdminReturns({
  ...
  const res = await api.get(`admin/orders/returns/?${params.toString()}`);

  return res.data;
}

export async function patchAdminReturn(returnId, body) {
  const res = await api.patch(`admin/orders/returns/${returnId}/`, body);
...
export async function patchAdminReview(reviewId, body) {
