...
export default function CancelOrderModal({
  ...
  const cancelLine =
    cancelTarget?.type === "line"
      ? order?.lines?.find((l) => l.id === cancelTarget.lineId)
      : null;

  const maxCancelQty = cancelLine?.cancellable_quantity ?? 1;

  const showQtyPicker = Boolean(cancelLine && maxCancelQty > 1);
  ...
  return (
    ...
    >
      ...
      {orderHasPaidDeliveryCharge(order) && cancelTarget?.type === "line" && (
