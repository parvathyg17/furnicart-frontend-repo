...
export default function OrderProgressStepper({ order }) {
  const tracker = resolveOrderTracker(order.status);

  const placed = formatDateShort(order.placed_at);

  return (
    <div className="odl-order-track">
      {tracker.cancelled && (
        <p className="odl-order-track-cancelled">This order was cancelled.</p>
      )}
      ...
      >
        {ORDER_TRACK_STEPS.map((step, i) => {
          const kind = orderStepDotKind(i, tracker);
          ...
          const meta =
            kind === "done" && i === 0 && order.placed_at ? placed : "";
              ...
              {i > 0 && (
                <div
                  className={`odl-track-bar${
                    orderBarFilled(i - 1, tracker.phase, tracker.allDelivered)
                      ? " odl-track-bar--on"
                      : ""
                  }`}
                  aria-hidden
