...
export default function OrderSuccessSummary({ order }) {
  const offerDiscountNum = Number(order.offer_discount_total ?? 0);

  const couponDiscountNum = Number(order.discount_total ?? 0);
  ...
  return (
    <section className="order-success-card" aria-label="Order summary">
      <h2 className="order-success-card-title">Order Summary</h2>
      ...
      {lines.length > 0 && (
        <div className="order-success-items">
          {lines.map((line) => {
            ...
            return (
              <div key={line.id} className="order-success-item">
                <div className="order-success-item-thumb-wrap">
                  {img ? (
                    ...
                    <div className="order-success-item-thumb-ph" aria-hidden />
                  )}
                </div>

                <div>
                  <p className="order-success-item-name">{line.product_name}</p>

                  {meta && <p className="order-success-item-meta">{meta}</p>}
      ...
      <dl className="order-success-lines">
        <div className="order-success-line">
          ...
          <dd>
            ...
            {formatMoney(
              offerDiscountNum > 0
                ? (order.subtotal_gross ?? order.subtotal)
                : order.subtotal,
            )}
          </dd>
        </div>
        ...
        {couponDiscountNum > 0 && (
          <div className="order-success-line order-success-line--savings">
            <dt>{order.coupon_code ? "Coupon" : "Discounts"}</dt>

            <dd>
              −₹
              {formatMoney(couponDiscountNum)}
            </dd>
          </div>
        )}
      </dl>
