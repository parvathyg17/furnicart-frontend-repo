...
export default function ProductRelatedGrid({ relatedProducts }) {
    ...
    >
      ...
      <div className="artisan-grid pd-user-related-grid pd-user-pdp-related-grid">
        {relatedProducts
          ...
          .filter((rp) => rp?.id != null)

          .map((rp) => {
            const rpSoldOut = isProductCardSoldOut(rp);

            const rpVariant = relatedDisplayVariant(rp);

            const rpPath = shopProductPathFrom(rp);
