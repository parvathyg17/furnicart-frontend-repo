...
export async function postAdminOffer(body) {
  const isFormData = body instanceof FormData;
  const config = isFormData
    ? { headers: { "Content-Type": "multipart/form-data" } }
    : {};

  const res = await api.post("admin/offers/", body, config);

  return res.data;
}

export async function patchAdminOffer(offerId, body) {
  const isFormData = body instanceof FormData;
  const config = isFormData
    ? { headers: { "Content-Type": "multipart/form-data" } }
    : {};

  const res = await api.patch(`admin/offers/${offerId}/`, body, config);
