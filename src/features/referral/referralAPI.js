...
export function captureReferralFromSearch(search) {
  ...
  if (ref) {
    sessionStorage.setItem(REFERRAL_TOKEN_KEY, ref.trim());
  }
}
...
export function setStoredReferralCode(code) {
  ...
  if (clean) {
    sessionStorage.setItem(REFERRAL_CODE_KEY, clean);
...
export function getStoredReferralPayload() {
  return {
    referral_token: sessionStorage.getItem(REFERRAL_TOKEN_KEY) || "",
    referral_code: sessionStorage.getItem(REFERRAL_CODE_KEY) || "",
  };
}
...
export async function fetchReferralMe() {
  const response = await api.get("promotions/referral/me/");
