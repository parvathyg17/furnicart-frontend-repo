...
function couponToForm(c) {
  return {
    ...
    min_order_subtotal: String(c.min_order_subtotal ?? "0"),
    max_discount_amount:
    ...
    max_redemptions_total:
      c.max_redemptions_total != null ? String(c.max_redemptions_total) : "",
    max_redemptions_per_user:
      c.max_redemptions_per_user != null
        ? String(c.max_redemptions_per_user)
        : "",
...
function buildPayload(form) {
  const body = {
    ...
    min_order_subtotal:
      form.min_order_subtotal === "" ? "0" : form.min_order_subtotal,
    is_active: form.is_active,
  };

  const maxDisc = (form.max_discount_amount || "").trim();

  body.max_discount_amount = maxDisc ? maxDisc : null;
  ...
  const parseRedemptionLimit = (raw) => {
  ...
  body.max_redemptions_total = parseRedemptionLimit(form.max_redemptions_total);
...
function discountLabel(c) {
  if (c.discount_type === "fixed") {
    return `₹${Number(c.discount_value).toLocaleString(undefined, {
...
export default function AdminCoupons() {
  ...
  const handleSave = async () => {
    ...
    if (form.discount_value === "" || form.discount_value == null) {
    ...
    try {
      if (modalMode === "create") {
        await postAdminCoupon(payload);

        toast.success("Coupon created.");
      } else {
        await patchAdminCoupon(editingId, payload);
      ...
      toast.error(
        formatProductApiError(err.response?.data) || "Could not save coupon.",
      );
    } finally {
      setSaveBusy(false);
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.error(
        formatProductApiError(err.response?.data) || "Could not delete coupon.",
      );
    } finally {
    ...
    <div className="admin-coupons-page">
      <header className="admin-coupons-header">
        <div>
          ...
          <p>
            Create and manage discount codes for checkout (customer apply flow
            comes next).
          </p>
        </div>
      ...
      {!loading && !error && coupons.length === 0 && (
        <p className="admin-coupons-muted">No coupons found.</p>
      )}
        ...
        <div className="admin-coupons-table-wrap">
          <table className="admin-coupons-table">
            ...
            <tbody>
              {coupons.map((c) => (
                <tr key={c.id}>
                  <td className="admin-coupons-code">{c.code}</td>

                  <td>{discountLabel(c)}</td>

                  <td>
                    {Number(c.min_order_subtotal).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </td>
        ...
        >
          ...
          >
            <h2 id="coupon-modal-title">
              {modalMode === "create" ? "New coupon" : "Edit coupon"}
            </h2>
            ...
            <div className="admin-coupons-field">
              ...
              <input
                ...
                onChange={onFormChange}
                placeholder={
                  form.discount_type === "percent" ? "e.g. 10" : "e.g. 500.00"
                }
              />
            </div>

            <div className="admin-coupons-field">
              <label htmlFor="cc-min">Minimum order subtotal</label>
            ...
            <div className="admin-coupons-field">
              <label htmlFor="cc-vf">Valid from (optional)</label>
            ...
            <div className="admin-coupons-field">
              <label htmlFor="cc-vu">Valid until (optional)</label>
            ...
            <div className="admin-coupons-field">
              <label htmlFor="cc-maxt">Max redemptions total (optional)</label>
            ...
            <div className="admin-coupons-field">
              <label htmlFor="cc-maxu">Max per user (optional)</label>
