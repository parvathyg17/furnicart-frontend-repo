...
function SalesChart({ breakdown, granularity }) {
  if (!breakdown?.length) {
    return <p className="da-muted">No sales in this period.</p>;
  }

  const revenues = breakdown.map((row) => Number(row.grand_total ?? 0));
  ...
  return (
    <div className="da-chart" role="img" aria-label="Sales revenue chart">
      {breakdown.map((row, index) => {
        const revenue = revenues[index];

        const barHeightPx = Math.max(
          Math.round((revenue / maxRevenue) * 180),
          revenue > 0 ? 12 : 0,
        );

        const barClass = [
          "da-chart-bar",
          index === peakIndex ? "is-peak" : "",
          index === highlightIndex ? "is-peach" : "",
        ]
          .filter(Boolean)
          .join(" ");

        return (
          <div key={row.date || row.label} className="da-chart-col">
            ...
            <span className="da-chart-label">
              {formatChartLabel(row.label, granularity)}
            </span>
          </div>
        );
      })}
...
function MetricCard({ label, value, icon: Icon, tone = "default" }) {
  return (
    <article className="da-metric-card">
      <div
        className={`da-metric-icon ${tone !== "default" ? `is-${tone}` : ""}`}
      >
      ...
      <div className="da-metric-body">
        ...
        <strong className="da-metric-value" title={value}>
          {value}
        </strong>
      </div>
    </article>
...
function TopProductsPanel({ rows }) {
  return (
    <section className="da-top-card">
      <div className="da-top-card-head">
        <h3>Top Products</h3>

        <span className="da-top-pill">Sorted by sales</span>
      </div>

      {!rows?.length ? (
        <p className="da-muted">No product sales in this window.</p>
      ) : (
...
function TopCategoriesPanel({ rows }) {
  ...
  return (
    <section className="da-top-card">
      ...
      {!rows?.length ? (
        <p className="da-muted">No category sales in this window.</p>
      ) : (
            ...
            return (
              <li key={`${row.id || row.name}-${index}`}>
                <div className="da-category-row">
                  ...
                  <span className="da-category-name">{row.name}</span>

                  <span className="da-category-meta">
                    ₹{formatMoney(revenue)} ({row.quantity_sold} sold)
                  </span>
                </div>
...
export default function AdminDashboard() {
  ...
  const loadAnalytics = useCallback(
    async ({ silent = false } = {}) => {
      ...
      try {
        const data = await fetchDashboardAnalytics(chartPeriod);

        const snap = stableStringify(data);

        if (silent && lastSigRef.current === snap) {
          return;
        }

        lastSigRef.current = snap;
  ...
  const handleLedgerExport = async (format) => {
    if (!analytics) {
      return;
    }
    ...
    try {
      const ledgerPeriod = chartPeriod === "monthly" ? "custom" : chartPeriod;

      await downloadAdminLedgerExport({
        period: ledgerPeriod,
        dateFrom: chartPeriod === "monthly" ? analytics.date_from : "",
        dateTo: chartPeriod === "monthly" ? analytics.date_to : "",
        format,
      });
      ...
      toast.error(err?.message || "Ledger export failed.");
    } finally {
      setExporting("");
    }
  };

  if (loadingLocal && !analytics) {
    return <div className="loading-cell">Loading dashboard...</div>;
  }
  ...
  const salesSummary = analytics?.sales_summary || {
    order_count: 0,
    subtotal_gross: "0.00",
    grand_total: "0.00",
  };
  ...
  return (
    <div className="da-page">
      <div className="dashboard-header da-header">
        <div>
          <span className="da-header-eyebrow">Overview</span>
        ...
        <div className="da-period-tabs">
          {CHART_TABS.map((tab) => (
            <button
              key={tab.id}
              type="button"
              className={chartPeriod === tab.id ? "is-active" : ""}
      ...
      <section className="da-chart-panel">
        <div className="da-panel-head">
          <div>
            ...
            {analytics?.date_from && (
              <p className="da-muted">
                Report period: {formatReportDate(analytics.date_from)} –{" "}
          ...
          <div className="da-ledger-actions">
            ...
            >
              <Download size={15} />
              {exporting === "pdf" ? "Exporting…" : "Ledger PDF"}
            </button>
            ...
            >
              <Download size={15} />
              {exporting === "excel" ? "Exporting…" : "Ledger Excel"}
            </button>
          </div>
        </div>

        <SalesChart
          breakdown={analytics?.sales_chart || []}
          granularity={analytics?.breakdown_granularity || "day"}
        />
      </section>

      <div className="da-top-grid">
        <TopProductsPanel rows={analytics?.top_products} />

        <TopCategoriesPanel rows={analytics?.top_categories} />
      </div>
    </div>
  );
}
