...
export default function AdminVariantMediaLibrary() {
  ...
  useEffect(() => {
    if (productId) {
      dispatch(getAdminProductDetail(productId));
    }
  }, [dispatch, productId]);
  ...
  const handleSaveImages = async () => {
    ...
    try {
      ...
      await dispatch(getAdminProductDetail(productId));

      setSelectedImages([]);
    } catch (error) {
      console.error(error);

      setUploadError(formatProductApiError(error) || "Upload failed");
    } finally {
  ...
  const handleDelete = async (imageId) => {
    ...
    try {
      ...
      setUploadError(
        formatProductApiError(payload) || "Failed to delete image",
      );
    }
  };
  ...
  if (productLoading || !variant) {
    return <div className="media-loading">Loading media library...</div>;
  }
    ...
    <div className="variant-media-page">
      ...
      <div className="media-header">
        <div>
          ...
          <p>Manage gallery imagery for {variant.variant_name}</p>

          <p className="media-minimum-hint">
            Each active variant must have at least three images saved before the
            product can go live. Currently saved on this variant:{" "}
        ...
        <div className="selected-preview-section">
          <div className="selected-preview-header">
            ...
            >
              ...
              {uploadLoading ? "Uploading..." : "Save Images"}
            </button>
          </div>
            ...
            {selectedImages.map((item) => (
              <div key={item.id} className="preview-card">
                ...
                <div className="preview-overlay">
                  <button onClick={() => setZoomedImage(item.preview)}>
                    <ZoomIn size={18} />
                  </button>

                  <button onClick={() => setCurrentCropImage(item)}>
                    <Crop size={18} />
                  </button>

                  <button onClick={() => removeSelectedImage(item.id)}>
                    <X size={18} />
                  </button>
                </div>
              </div>
          ...
          <div key={image.id} className="media-card">
            ...
            <div className="media-info">
              ...
              <div className="media-actions">
                <button
                  onClick={() => setZoomedImage(image.image_url || image.image)}
                >
                  <ZoomIn size={18} />
                </button>
