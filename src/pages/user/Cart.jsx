...
function cartLineImageUrl(variant) {
...
export default function Cart() {
  ...
  const load = useCallback(
    async ({ silent = false } = {}) => {
      ...
      try {
        ...
        const snap = stableStringify(res);

        dispatch(setCartItemCount(res.item_count));

        if (silent && lastCartSigRef.current === snap) {
          return;
        }

        lastCartSigRef.current = snap;
  ...
  const changeQty = async (itemId, nextQty, maxQ) => {
    setCheckoutErr(null);

    try {
      await updateCartItemApi(itemId, nextQty);
  ...
  const remove = async (itemId) => {
    ...
    try {
      ...
      lastCartSigRef.current = stableStringify(res);
      ...
      setError(
        formatProductApiError(err.response?.data) || "Could not remove item.",
      );
    }
  };
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setCheckoutErr(
        formatCheckoutValidationError(body) ||
          formatProductApiError(body) ||
      ...
      <main className="artisan-main-wrap cart-bag-main">
        <header className="cart-bag-page-head">
          <div className="cart-bag-page-head-text">
            <h1 className="cart-bag-page-title artisan-font-serif">Cart</h1>
          ...
          <Link className="cart-bag-continue" to="/shop">
            Continue shopping
          </Link>
        </header>
        ...
        {loading ? (
          <p className="cart-bag-muted">Loading cart…</p>
        ) : !data?.items?.length ? (
          <div className="cart-bag-empty">
            ...
            <Link className="cart-bag-empty-cta" to="/shop">
              Browse shop
            </Link>
          </div>
        ) : (
          <div className="cart-bag-layout">
            <div className="cart-bag-col-items">
              <ul className="cart-bag-list">
                {data.items.map((row) => {
                  const maxQ =
                    typeof row.max_quantity === "number"
                      ? row.max_quantity
                      : Math.min(row.variant?.stock || 0, 10);

                  const avail = row.line_availability;

                  const lineProblem =
                    avail && avail.status !== "ok" && avail.message;

                  const v = row.variant;

                  const imgUrl = cartLineImageUrl(v);
                  ...
                  if (!pills.length && (v?.variant_name || "").trim()) {
                    pills.push({
                      key: "v",
                      label: String(v.variant_name).trim(),
                    });
                  }

                  return (
                    <li key={row.id} className="cart-bag-row">
                      <div className="cart-bag-row-grid">
                        ...
                        <div className="cart-bag-mid">
                          ...
                          {pills.length > 0 && (
                            <div className="cart-bag-pills">
                              {pills.map((p) => (
                                <span key={p.key} className="cart-bag-pill">
                                  {p.label}
                                </span>
                              ))}
                            </div>
                          ...
                          {row.quantity >= maxQ && (
                            <p className="cart-bag-qty-cap-note">
                              Maximum {maxQ} for this item (cart limit or
                              stock).
                            </p>
                          )}
                        ...
                        <div className="cart-bag-side">
                          <p className="cart-bag-line-price artisan-font-serif">
                            ₹{formatCartMoney(row.line_subtotal)}
                          </p>
              ...
              <div className="cart-bag-summary-card">
                ...
                <dl className="cart-bag-summary-lines">
                  ...
                  {Number(data.offer_discount_total) > 0 && (
                    <div className="cart-bag-summary-line">
                      <dt>Subtotal</dt>
                      <dd>₹{formatCartMoney(data.subtotal)}</dd>
                    </div>
                  )}
                  ...
                  <div className="cart-bag-summary-line">
                    ...
                    <dd className="cart-bag-summary-muted">At checkout</dd>
                  </div>
                ...
                <button
                  ...
                  disabled={
                    checkoutBusy || !data?.items?.length || !data?.can_checkout
                  }
                ...
                {!data.can_checkout && (
                  <p className="cart-bag-summary-note">
                    ...
                    availability). Update quantities or remove lines, then try
                    again.
                  </p>
                )}
