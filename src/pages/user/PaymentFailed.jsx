...
function FailedIllustration() {
  return (
    ...
    >
      <defs>
        <linearGradient id="chk-fail-bg" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#f5ebe8" />

          <stop offset="100%" stopColor="#ead9d4" />
        </linearGradient>
      </defs>
...
function resolveCopy(reason, message) {
  if (reason === "cancelled") {
    return {
...
export default function PaymentFailed() {
  ...
  const { title, lead } = resolveCopy(reason, state.message);
  ...
  return (
    <div className="artisan-shop checkout-shell">
      <main className="checkout-success-wrap checkout-payment-failed-wrap">
        ...
        <p className="checkout-success-lead">{lead}</p>
        ...
        <div className="checkout-success-actions">
          <Link className="checkout-btn-primary" to="/checkout">
            Try checkout again
          </Link>

          <Link className="checkout-btn-secondary" to="/cart">
            View cart
          </Link>

          <Link className="checkout-btn-secondary" to="/shop">
            Continue shopping
          </Link>
        </div>
      </main>
