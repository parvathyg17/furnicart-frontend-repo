...
export default function WalletPage() {
  ...
  const loadWallet = useCallback(async () => {
    try {
      ...
      setError(
        formatProductApiError(err.response?.data) || "Could not load wallet.",
      );
    }
  }, []);
  ...
  const loadTransactions = useCallback(
    async (page = 1, type = "") => {
      ...
      try {
        const data = await fetchWalletTransactionsApi({
          page,
          pageSize: 10,
          type,
        });
    ...
    <AccountLayout>
      <div className="wallet-page">
        <header className="wallet-page-header">
          <h1 className="wallet-page-title artisan-font-serif">My Wallet</h1>
        ...
        {error && (
          <div className="wallet-banner wallet-banner--error" role="alert">
            {error}
          </div>
        )}
          ...
          <>
            ...
            <section className="wallet-txn-section">
              <div className="wallet-txn-head">
                <h2 className="wallet-txn-title">Transaction history</h2>
              ...
              {txnLoading && transactions.length === 0 ? (
                <p className="wallet-muted">Loading transactions…</p>
              ) : !transactions.length ? (
                ...
                <ul className="wallet-txn-list">
                  {transactions.map((txn) => (
                    <li key={txn.id} className="wallet-txn-item">
                      <div
                        className={
                          txn.type === "credit"
                      ...
                      <div className="wallet-txn-amounts">
                        ...
                        >
                          {txnSign(txn.type)}₹{formatMoney(txn.amount)}
                        </p>
              ...
              {pagination.total_pages > 1 && (
                <div className="wallet-pagination">
                  <button
                    ...
                    disabled={pagination.current_page <= 1 || txnLoading}
                    onClick={() => {
