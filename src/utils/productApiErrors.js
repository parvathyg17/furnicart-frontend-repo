...
export function formatProductApiError(payload) {
  ...
  if (
    ...
    const nested = formatProductApiError(payload.detail);

    if (nested && nested !== "Something went wrong.") {
      return nested;
    }
  }
  ...
  for (const [key, val] of Object.entries(payload)) {
    if (["error", "detail", "status", "message"].includes(key)) {
      continue;
    }
...
export function mapPayloadToFormErrors(payload) {
  ...
  for (const [key, val] of Object.entries(payload)) {
    if (["error", "detail", "status"].includes(key)) {
      continue;
    }
    ...
    if (key === "message" && typeof val === "string" && val.trim()) {
  ...
  if (!next._general) {
    ...
    if (typeof detail === "string") {
      ...
      next._general =
        typeof detail[0] === "string" ? detail[0] : JSON.stringify(detail[0]);
    }
  }

  const fieldKeys = Object.keys(next).filter((key) => key !== "_general");

  if (!next._general && fieldKeys.length === 0) {
